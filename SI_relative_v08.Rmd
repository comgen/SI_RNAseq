...
modelset_t$SEX <- str_replace(modelset_t$SEX, "Male", "1")
modelset_t$SEX <- str_replace(modelset_t$SEX, "Female", "2")
modelset_t$SEX <- as.numeric(modelset_t$SEX)
...
modelset_t <- cbind(Ethnicity_code = info$Ethnicity_code[match(rownames(modelset_t), info$SampleID)], modelset_t) #remove for BipSeq
#MZ: Is Ethnicity_code numerical or a factor variable? Should be a factor. Are RIN and PMI continuous variables?
...
#MZ: Note that cv.glmnet should also contain the same penalty factor as the final model. Otherwise, you will select a sub-optimal lambda (which will be optimised for the model without mandatory clinical covariates, while you want the lambda optimised for the model with those covariates):

      lasso.cv.tr <- cv.glmnet(x=modelset_t[,!colnames(modelset_t) %in% gene], y = modelset_t[,colnames(modelset_t) %in% gene], alpha=1, type.measure="deviance",
  ...
  #    lasso.lam.tr <- lasso.cv.tr$lambda.min #optimal lambda (minimal)
      lasso.lam.tr <- lasso.cv.tr$lambda.1se #optimal lambda 
      ...
      #MZ: As an alternative, you can pull out the fitted final model directly from cv.glmnet, by selecting the model with the right lambda from lasso.mod.tr = lasso.cv.tr$glmnet.fit.
...
testset_t$SEX <- str_replace(testset_t$SEX, "Male", "1")
testset_t$SEX <- str_replace(testset_t$SEX, "Female", "2")
testset_t$SEX <- as.double(testset_t$SEX)
...
testset_t <- cbind(Ethnicity_code = info$Ethnicity_code[match(rownames(testset_t), info$SampleID)], testset_t) #remove for BipSeq
...
#MZ: Note that this R2 (= fraction of the null deviance explained by the deviance, i.e. dev.ratio=1-dev/nulldev) will be the R2 on the training data and thus too optimistic. The corresponding R2 on the test data will potentially be much smaller.
...
#subset <- subset[!subset %in% c( "CNNM4", "FKBP2")]
...
#model_fraction <- length(HC$SampleID) *2/3 #for BipSeq sACC, Amg
...
#MZ: Why is Study commented out, i.e. why are you not adjusting for Study in this model?
...
#MZ: Again, this is on the same model where the data was fitted. But here this is probably not such a big problem, because the model only has a small number of covariates, so the danger of over-fitting is much smaller.
...
p_white <- 
 ...
 ggsave(plot=p, path = dir, filename = "compare_WSR_BD_HC_redstrip_BD_model.pdf", width = 9, height = 10.48, units = "cm")
  ggsave(plot=p_blue, path = dir, filename = "compare_WSR_BD_HC_bluestrip_BD_model.pdf", width = 9, height = 10.48, units = "cm")
  ggsave(plot=p_white, path = dir, filename = "compare_WSR_BD_HC_whitestrip_BD_model.pdf", width = 9, height = 10.48, units = "cm")
...
ggsave(plot =auc_increasing_genes_sWSR_all, filename = paste0(save_dir, "auc_increasing_genes_sWSR_all.pdf"), width = 43, height = 43, units = "mm" )
...
#facet_wrap(~ factor(gene_id, levels=genes_R2$gene_id), nrow  = 1) +
  ...
  scale_colour_manual(values=c(cols[1], cols[2]), labels= c("HC1", "HC2")) +
