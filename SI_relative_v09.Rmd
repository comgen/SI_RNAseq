...
```{r change subset, eval=FALSE}

PGC3_PIP_070 <- read_tsv("/Users/asbjorh/PhD/RNAseq_temporal/paper/revision/gene_pip070_PGC3.txt", col_names = FALSE) 

subset <- PGC3_PIP_070$X1
subsetname <- "PGC3_PIP_070"
```
...
data_dir <- "/Users/asbjorh/PhD/RNAseq_DE/"
...
rpkm_log_CMC_Pitt <-  read_tsv("/Users/asbjorh/PhD/RNAseq_DE/CMC-Pitt_rpkm_log.txt")
rpkm_log_BipSeq_sACC <- read_tsv("/Users/asbjorh/PhD/RNAseq_DE/BipSeq-sACC_rpkm_log.txt")
rpkm_log_GVEX_SMRI <- read_tsv("/Users/asbjorh/PhD/RNAseq_DE/GVEX-SMRI_rpkm_log.txt")

info_all <-  read_tsv("/Users/asbjorh/PhD/RNAseq_DE/info_all.txt")
...
̛
```{r change save dir}
dir <- paste0("/Users/asbjorh/PhD/RNAseq_relative/4_cortical_sets/", subsetname, "/lasso_cv_lambda-1se_fixed_covarites/", "fix_lambda/", dataset, "/WSR/")
```
...
dir <- paste0("/Users/asbjorh/PhD/RNAseq_relative/4_cortical_sets/", subsetname, "/lasso_cv_lambda-1se_fixed_covarites/", "fix_lambda/", dataset, "/sWSR/")
```
...
```{r plot sWSR DE}
order.CMC_HBCC.gene_id <- read.csv("/Users/asbjorh/PhD/RNAseq_relative/4_cortical_sets/high_l2g_closest/order.CMC_HBCC.gene_id.txt") #as calculated from correlation matrix in DE_absolute_v08.Rmd
...
Look for batch effects in sWSR values between sets
```{r PCA sWSR}
sWSR_CMC_HBCC <- read_tsv("/Users/asbjorh/PhD/RNAseq_relative/4_cortical_sets/high_l2g_closest/lasso_cv_lambda-1se_fixed_covarites/CMC-HBCC/sWSR/sWSR.txt")
sWSR_CMC_Pitt <- read_tsv("/Users/asbjorh/PhD/RNAseq_relative/4_cortical_sets/high_l2g_closest/lasso_cv_lambda-1se_fixed_covarites/CMC-Pitt/sWSR/sWSR.txt")
sWSR_GVEX_SMRI <- read_tsv("/Users/asbjorh/PhD/RNAseq_relative/4_cortical_sets/high_l2g_closest/lasso_cv_lambda-1se_fixed_covarites/GVEX-SMRI/sWSR/sWSR.txt")
...
#      scale_color_manual(values = cols)
   ...
   dir <- "/Users/asbjorh/PhD/RNAseq_relative/4_cortical_sets/high_l2g_closest/lasso_cv_lambda-1se_fixed_covarites/"
...
Look for batch effects in expression values between sets
...
#      scale_color_manual(values = cols)
   scale_color_brewer(palette = "Dark2") 
    ...
    ggsave(plot= PCA_rpkm_subset_set, filename = paste0(dir, "PCA_rpkm_subset_set.pdf"), width = 89, height = 89, units = "mm")
        ...
        ggsave(plot= PCA_rpkm_subset_set, filename = paste0("/Users/asbjorh/PhD/RNAseq_relative/4_cortical_sets/high_l2g_closest/lasso_cv_lambda-1se_fixed_covarites/", "PCA_rpkm_subset_res_set.pdf"), width = 89, height = 89, units = "mm")
...
```{r import control experiment model with cases}

SI <- read_tsv("/Users/asbjorh/PhD/RNAseq_relative/4_cortical_sets/high_l2g_closest/lasso_cv_lambda-1se_fixed_covarites/BD_model/CMC-HBCC/SI_C_BD_wsr.txt")
...
info_all$Ethnicity <- str_replace(info_all$Ethnicity,  pattern = "Caucasian", replacement = "CAUC") 
info_all$Ethnicity <- str_replace(info_all$Ethnicity,  pattern = "Hispanic", replacement = "HiSP")
info_all$Ethnicity <- str_replace(info_all$Ethnicity,  pattern = "AS", replacement = "Asian")

info_all$Ethnicity <- factor(info_all$Ethnicity, levels = c("CAUC",             "HiSP" ,            "Asian" ,     "African-American" ,       "Unknown"          ))
...
SI <-  cbind(Ethnicity = info_all$Ethnicity[match(SI$SampleID, info_all$SampleID)], SI)
#SI <-  cbind(Sex = info_all$ReportedGender[match(SI$SampleID, info_all$SampleID)], SI)
#SI <-  cbind(Age = info_all$Age[match(SI$SampleID, info_all$SampleID)], SI)
#SI <-  cbind(Study = info_all$Study[match(SI$SampleID, info_all$SampleID)], SI) #For BrainGVEX


SI_matrix <- model.matrix(~ Ethnicity, -1, data = as.data.frame(SI) )
SI_dummy <- data.frame(SI[ , ! colnames(SI) %in% "Ethnicity"],       # Create dummy data
                         model.matrix( ~ Ethnicity - 1, as.data.frame(SI)))


SI_glm <- glm(diag ~ SI, data= as.data.frame(SI), family = "binomial")
SI_glm_cov <- glm(diag ~ SI + Ethnicity + Sex + Age,
       #         +Study, #For BrainGVEX
                  data= as.data.frame(SI), family = "binomial") #check to see which variables effect outcome
