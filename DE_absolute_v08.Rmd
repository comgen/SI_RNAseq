...
high_L2G <- read_tsv("~/RNAseq_temporal/genes_of_interest/OpenTargets/genes_l2g_0.5_locus.txt")
high_L2G_closest <- read_tsv("~/RNAseq_temporal/genes_of_interest/OpenTargets/loci_missing_nearest.txt")
...
#genes_res_diff <- read_tsv("~/RNAseq_temporal/analysis/OFC/core_genes_consistent_res_diff.txt", col_names = FALSE)
...
```{r import count data}
count_GVEX <- read.csv("~/RNAseq_DE/count_GVEX.txt")
...
BipSeq_info <- read_xlsx("~/RNAseq_temporal/data/BipSeq/amygdala_sACC/counts/BipSeq_Metadata-distr20221110.xlsx")
library(readxl)
BipSeq_info <- read_xlsx("~/RNAseq_temporal/data/BipSeq/amygdala_sACC/counts/BipSeq_Metadata-distr20221110.xlsx")
...
info_GVEX_seq <- read_tsv("~/Psychencode/data/SYNAPSE_METADATA_MANIFEST.tsv")
info_GVEX_seq <- info_GVEX_seq %>% select( individualID, tissue, RIN, BrodmannArea, hemisphere, RIN, pH)
info_GVEX_clin <- read_csv("~/Psychencode/individual_BrainGVEX_metadata.csv")
...
info_GVEX %>%  head()
info_CM_seq <- read_csv("~/RNAseq_temporal/data/CommonMind_release3/CMC_Human_rnaSeq_metadata.csv")
...
info_CM_clin <- read_csv("~/RNAseq_temporal/data/CommonMind_release3/CMC_Human_clinical_metadata.csv")
...
clin_info_cm <- read_delim("~/RNAseq_temporal/data/CommonMind_release3/CMC_Human_clinical_metadata.csv",
...
##CORE##
##GRAPH##
Gene expression principal components analysis(PCA). This produces the plot for Figure S1.
...
ggsave(PCA_CMC_HBCC, filename = "~/RNAseq_DE/4_cortical_sets/PCA_CMC_HBCC.pdf", height = 8, width = 8, units = "cm")
```
...
ggsave(PCA, filename = "~/RNAseq_DE/4_cortical_sets/PCA_CMC_Pitt.pdf", height = 4, width = 4, units = "cm")
ggsave(PCA, filename = "~/RNAseq_DE/4_cortical_sets/PCA_GVEX_SMRI.pdf", height = 4, width = 4, units = "cm")
ggsave(PCA, filename = "~/RNAseq_DE/4_cortical_sets/PCA_BipSeq_sACC.pdf", height = 4, width = 4, units = "cm")
...
##ADDITIONAL##
Simplified analysis looking for differential expression differences between cases and controls. It is a basic wilcoxon differences in mean, and is not in the paper.
...
# Making into long format: 3  basic columns (gene, sample, expression) + other info (sample set, diagnosis?)
...
##CORE##
##GRAPH##
This produces the heatmaps of gene expression, and identifies clusters of coexpressed genes. The plots produced here make up Figure 2.
```{r  compare heatmaps all }
library(dendextend) #clustering
library(gplots) #for heatmaps
library(corrplot) #clustering

#heatmap.compare() is a function which code is found in functions.R

dir <- "~/RNAseq_relative/4_cortical_sets/high_l2g_closest/CMC-HBCC/correlation/"
...
write.csv(order.CMC_HBCC.gene_id, file = "~/RNAseq_relative/4_cortical_sets/high_l2g_closest/order.CMC_HBCC.gene_id.txt")


dir <- "~/RNAseq_relative/4_cortical_sets/high_l2g_closest/CMC-Pitt/correlation/"
...
dir <- "~/RNAseq_relative/4_cortical_sets/high_l2g_closest/GVEX-SMRI/correlation/"
pdf(file = paste0(dir, "high_L2g_heatmap_compare.pdf"))
heatmap.compare(rpkm_log_GVEX_SMRI[rownames(rpkm_log_GVEX_SMRI) %in% subset,] , order = order.CMC_HBCC.gene_id)
dev.off()


dir <- "~/RNAseq_relative/4_cortical_sets/high_l2g_closest/BipSeq-sACC/correlation/"
pdf(file = paste0(dir, "high_L2g_heatmap_compare.pdf"))
...
dir <- "~/RNAseq_DE/4_cortical_sets/high_l2g_closest/"
```


##ADDITIONAL##
analyse expression levels standardised with mean and sd of controls
...
##CORE##
Analyse expression levels after regressing out the known covariates. This way, the expression analysis "absolute expression" is as comparable as possible to the modelled relative expression. The residualised expression values are also standardised with control mean and sd. 
...
##CORE##
Use the residualised and standardised expression values to predict diagnosis. Model in three datasets and cross-validate in the fourth. 
##GRAPH##
These results are found in Table 2 (right panel), AUC and p.
...
```
##GRAPH##
The resulting AUC plots are found in Figure 6f
...
##GRAPH##
These residualised and standardised absolute expression levels are shown in Figure 1a
...
##CORE##
##GRAPH##
The following produces the density plots of predicted diagnosis found in Figure 6e, that are based on absolute expression. The R2 on the liability scale, r2l, are found in Table2, rightmost panel.
...
##CORE##
##GRAPH##
The following visualises the differential expression based on residualised and standardised expression values. The plot is found in Figure 1b.

```{r plot DE of residualised expression values}
rpkm_residual_subset_all_std <- read_tsv("~/RNAseq_DE/4_cortical_sets/residualised_expression/high_l2g_closest/rpkm_residual_subset_all_std.txt")
dir <- "~/RNAseq_DE/4_cortical_sets/residualised_expression/high_l2g_closest/"
...
##CORE##
DE analysis with LIMMA-VOOM (IA). This is a standard RNAseq DE expression analysis. The only difference from previous studies by others is that we only look at the BD GWAS l2g-genes, and therefore do not apply correction for multiple testing on the entire transcriptome. The result is visualised in Figure 1c.


```{r work on limma results IA}

list.limma <- list.files(path = "~/RNAseq_DE/Ibrahim/", pattern = "^de_limma", full.names = TRUE)
...
limma_DE$Set <- #remove unnescessary prefix text from set names
  str_remove(limma_DE$Set, "~/RNAseq_DE/Ibrahim//de_limma_") 
